...
- `src/App.tsx` contains the full resume page markup.
- `src/components/` contains small presentational components.
- `src/index.css` is the Tailwind entrypoint and global token file.
- `src/App.css` contains the page-specific background gradient.
...
- Let ESLint own formatting; there is no Prettier config.
